...
export default function DashboardLayout({ children }) {
  ...
  if (loading) {
    return (
      <div className="h-screen w-screen flex items-center justify-center bg-[#0F172A] text-white font-sans">
  ...
  if (!user) {
    return (
      <div className="h-screen w-screen flex items-center justify-center bg-[#0F172A] text-white p-4 font-sans">
  ...
  return (
    <div className="h-screen w-screen overflow-hidden flex flex-col md:flex-row bg-[#F8FAFC] font-sans text-slate-800">
      ...
      <div className="md:hidden h-14 bg-[#0F172A] text-white px-4 flex items-center justify-between border-b border-slate-800 shrink-0 z-30">
        <Link href="/" className="flex items-center space-x-2">
          ...
          <span className="font-bold text-base text-white tracking-tight">DentalFlow™</span>
        </Link>
        <button
      ...
      <aside
        className={`fixed md:relative inset-y-0 left-0 z-40 w-64 bg-[#0F172A] text-white flex flex-col justify-between border-r border-slate-800 transition-transform duration-200 ease-in-out md:translate-x-0 ${
      ...
      >
        ...
        <div className="p-4 border-t border-slate-800 shrink-0">
          <button
            onClick={logout}
            className="w-full flex items-center space-x-2 text-xs text-slate-400 hover:text-red-400 transition-colors p-2 rounded-lg hover:bg-slate-800 focus:outline-none"
          >
      ...
      {/* Main Right Body - Independent Scroll Area */}
      <div className="flex-1 flex flex-col h-full min-w-0 overflow-hidden">
        ...
        <header className="h-14 bg-white border-b border-slate-200 px-4 md:px-6 flex items-center justify-between shrink-0 z-20">
          ...
          <div className="flex items-center space-x-4">
            <button onClick={logout} className="text-xs text-slate-500 hover:text-slate-800 font-semibold focus:outline-none">
              Sign Out
            </button>
        ...
        {/* Dynamic Content Body (Independent Scroll Area) */}
